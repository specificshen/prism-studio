...
**一切影响最终像素的、ISV 在 Blender 里能感知的效果，都必须进 schema。**

- ISV 在 Blender 里调过的：灯光能量/颜色、材质 PBR 参数、相机镜头、世界 HDRI/天空、色彩管理曝光。
- ISV 期望「我在 Blender 里看到的就是前端样子」的：玻璃、镀膜、自发光、雾、大气。
- 甲方与 ISV 会反复拉扯微调的：bloom 强度、AO、色调映射、阴影。

只要满足「ISV 需要控制它」或「交付验收时它会被对比」，它就没有资格留在代码里。
...
- ❌ 它不是藏参数的地方。「懒得进 schema」不是放 `EDITOR_DEFAULTS` 的理由——ISV 要控制的必须进 schema。

## 反面教材：旧工程 17 类硬编码效果

旧工程（threejs-webgpu-editor-core）把以下效果全部硬编码在前端或 Python 生成器里，导出的 JSON 根本没有这些字段，导致「Blender 里调了前端不变、前端改代码 ISV 不知道」：
...
1. **ISV 需要控制它吗？**（他在 Blender 里能感知、会要求调，或验收时会被对比）→ 是：**进 schema**，走 `scene-schema-evolution` 六步流程。
